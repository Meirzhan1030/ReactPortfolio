...
class AdmProjects extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      error: null,
      ...
      return (
        <div>
          <div style={{ display: 'flex', justifyContent: "space-between", alignItems: "center" }}>
            ...
            <Link className="btn-add" to="/create/project">Add new</Link>
          </div>
          <div className="card animated fadeInDown" >
            <table>
              ...
              {posts.map(post => (
                <tr key={post.id}>
                  <td>{post.title}</td>
                  <td>{post.description.length > 30 ? post.description.slice(0, 30) + '...' : post.description}</td>
                  <td>{post.url_name}</td>
                  <td>{post.url}</td>
                  <td>
                    <img src={post.image} alt="Post" width={200} onError={(e) => { e.target.onerror = null; e.target.src="https://via.placeholder.com/100"; }}/>
                    <br/><br/>
                  </td>
                  <td>{post.created_at}</td>
                  <td>
                    <Link className="btn-edit" to={{ pathname: "/banner/post/" + post.id }}>Edit</Link>
                    &nbsp;
